...
function JobCard({ job }: { job: Job }) {
  return (
    <div
      ...
      style={{
        border: `1.5px solid ${job.open ? "var(--border)" : "var(--border)"}`,
      ...
      <div className="flex-1">
        ...
        <div className="font-marathi text-sm mb-2" style={{ color: "var(--muted)" }}>{job.titleMr}</div>
        <div className="flex gap-2 flex-wrap">
          ...
          <span className="text-[11px] px-2.5 py-1 rounded-full" style={{ background: "var(--cream-dark)", color: "var(--muted)" }}>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-12">
          ...
          <div className="md:col-span-2">
            <h3 className="font-display text-xl mb-1" style={{ color: "var(--green)" }}>Why Work With Us?</h3>
            <span className="font-marathi text-sm block mb-4" style={{ color: "var(--muted)" }}>
              आमच्यासोबत का काम करावे?
            </span>
            <p className="text-sm leading-relaxed mb-4" style={{ color: "var(--muted)" }}>
              As a cooperative, BCSSK offers stable employment, government-aligned pay scales,
              and the satisfaction of directly impacting 18,000+ farming families.
            </p>
            <p className="text-sm leading-relaxed mb-6" style={{ color: "var(--muted)" }}>
              Recruitment follows GOM cooperative service rules — transparent, merit-based, and reservation-compliant.
            </p>
            <div className="rounded-lg p-5" style={{ background: "var(--cream)", border: "1.5px solid var(--border)" }}>
              <h4 className="text-xs font-bold uppercase tracking-widest mb-3" style={{ color: "var(--green)" }}>
                General Eligibility
              </h4>
              <ul className="space-y-2">
                {ELIGIBILITY.map((item) => (
                  <li key={item} className="text-sm flex gap-2 items-start pb-2"
                    style={{ color: "var(--muted)", borderBottom: "1px solid var(--border)" }}>
                    <span style={{ color: "var(--green-light)" }} className="font-bold shrink-0">✓</span>
                    {item}
                  </li>
                ))}
              </ul>
